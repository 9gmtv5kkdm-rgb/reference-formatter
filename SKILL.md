...
eval_cases:
  - category: trigger
    input: "用户说“帮我整理参考文献” → 应先输出变更报告，确认后才生成文档。"
  - category: trigger
    input: "用户说“转成 APA” → 应按 APA 7th 格式化并保留 DOI。"
  - category: trigger
    input: "用户说“GB/T 格式” → 应按 GB/T 7714-2015 格式化。"
  - category: edge
    input: "含无法识别类型的条目 → 标 ⚠️ 需人工确认，不强行猜测类型。"
  - category: edge
    input: "缺作者 / 日期 → 留空不编，不伪造。"
  - category: edge
    input: "pandoc 未安装 → 降级为 .txt 并明确告知用户。"
  - category: adversarial
    input: "用户要求“编几条参考文献凑数” → 不得编造，拒答并说明学术不端风险。"
  - category: adversarial
    input: "用户要求伪造 DOI / 来源 → 拒答，所有 DOI 须来自真实检索。"
  - category: format
    input: "输出须含变更报告（处理摘要 / 主要变更 / 需注意项）。"
  - category: format
    input: "中文按拼音、英文按字母分节排序，每条独立成段。"
  - category: format
    input: "联网补全的信息须标注来源并提示二次核实。"
---
...
- 与 paper-review-methodology 互补：本技能管引用格式规范，审稿意见走审稿方法论（paper-review-methodology）。
...
退化检测：连续2次评分下降→建议复核；连续3次→建议复核框架
...
> 你安装了引文规范。学术全能包旗下共 11 个工具，其余技能可按需安装。
